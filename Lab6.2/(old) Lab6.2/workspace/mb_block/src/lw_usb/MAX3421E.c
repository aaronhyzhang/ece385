...
//	xil_printf("Entering MAXreg_wr\n");
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01); //select
	int status;
	BYTE array_send [2] = {reg + 2, val};

	status = XSpi_Transfer(&SpiInstance, array_send, NULL, 2);
	if(status != 0) {
		xil_printf("Error: MAXreg_wr");
	}

	XSpi_SetSlaveSelect(&SpiInstance, 0x00); //deselect
}
...
//	xil_printf("Entering MAXbytes_wr\n");
	//psuedocode:
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01); //select
	int status;
	BYTE array_send [nbytes+1];
	array_send[0] = reg +2;
//	memset(&array_send[1], data, nbytes);
	for(int i =0; i<nbytes; i++) {
		array_send[i+1] = data[i];
	}


	status = XSpi_Transfer(&SpiInstance, array_send, NULL, nbytes+1);
	if(status != 0) {
...
//	xil_printf("Entering MAXreg_rd\n");
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01); //select
	int status;
	BYTE array_send [2] = {reg, 0x00};
	BYTE array_rec [2];

	status = XSpi_Transfer(&SpiInstance, array_send, array_rec, 2);
	if (status != 0) {
		xil_printf("Error: MAXreg_rd - Transfer reg");
	}
	BYTE val = array_rec[1];
...
//	xil_printf("Entering MAXbytes_rd");
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	//write reg via SPI
	...
	BYTE array_send [nbytes+1];
	array_send[0] = reg;
//	memset(&array_send[1], 0, nbytes);
	BYTE array_rec [nbytes+1];

	status = XSpi_Transfer(&SpiInstance, array_send, array_rec, nbytes+1);
	if(status != 0) {
		xil_printf("Error: MAXbytes_rd - Transfer reg");
	}

	for(int i =0; i<nbytes; i++) {
		data[i] = array_rec[i+1];
	}
//	memcpy(data, &array_rec[1], nbytes);
