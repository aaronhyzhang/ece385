...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE rec;
	send = reg + 2;
	status = XSpi_Transfer(&SpiInstance, &send, &rec, 1);
	if(status != 0) {
		xil_printf("Error: MAXreg_wr - Transfer reg");
	}

	send = val;
	status = XSpi_Transfer(&SpiInstance, &send, &rec, 1);
	if(status != 0) {
			xil_printf("Error: MAXreg_wr - Transfer val");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	status = XSpi_Transfer(&SpiInstance, &send, &rec, 1);
	if(status != 0) {
			xil_printf("Error: MAXbytes_wr - Transfer reg");
	}
	...
	for (i = 0; i < nbytes; i++) {
		...
		status = XSpi_Transfer(&SpiInstance, &send, &rec, 1);
		if(status != 0) {
				xil_printf("Error: MAXbytes_wr - Transfer data[%d]", i);
		}
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	status = XSpi_Transfer(&SpiInstance, &send, &rec, 1);
	if (Status != 0) {
		xil_printf("Error: MAXreg_rd - Recieve val");
	}
...
//		rec = data[i];
		send = 0x00;
		status = XSpi_Transfer(&SpiInstance, &send, &data[i], 1);
		if(status != 0) {
			xil_printf("Error: MAXbytes_wr - Transfer data[%d]", i);
		}
	}
